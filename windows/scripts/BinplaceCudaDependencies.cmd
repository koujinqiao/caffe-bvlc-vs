...
if %IS_CPU_ONLY_BUILD% == true (
    ...
    if %USE_CUDNN% == true (
        ...
        if "%CUDNN_PATH%" == "" (
		    if exist "%CUDA_TOOLKIT_BIN_DIR%\cudnn*.dll" copy /y "%CUDA_TOOLKIT_BIN_DIR%\cudnn*.dll" "%OUTPUT_DIR%" else copy /y "%CUDNN_PATH%\cuda\bin\cudnn*.dll" "%OUTPUT_DIR%"

        ) else (
		    if exist "%CUDNN_PATH%\cuda\bin\cudnn*.dll" copy /y "%CUDNN_PATH%\cuda\bin\cudnn*.dll" "%OUTPUT_DIR%" else copy /y "%CUDA_TOOLKIT_BIN_DIR%\cudnn*.dll" "%OUTPUT_DIR%"
        )
    ) else (
